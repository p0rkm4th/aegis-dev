...
PYTHONPATH="$install_dir" python -c 'from aegis.migrations import validate_migrations; migrations = validate_migrations(); assert migrations and migrations[-1] == "017_conversations.sql"'
...
PYTHONPATH="$sdist_site_dir" python -c 'from aegis.migrations import validate_migrations; assert validate_migrations()[-1] == "017_conversations.sql"'
