...
def test_deterministic_calendar_conflict_request_uses_read_only_action():
    manager = PackManager()
    bundle = next(bundle for bundle in reference_bundles() if bundle.manifest.pack_id == "calendar")
    manager.discover(bundle)
    manager.install("calendar", frozenset({"calendar.read", "calendar.write"}))
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Show my calendar conflicts.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
...
def test_deterministic_chore_report_uses_household_workspace_composition():
    manager = PackManager()
    bundle = next(
        bundle for bundle in reference_bundles() if bundle.manifest.pack_id == "household-reports"
    )
    manager.discover(bundle)
    manager.install("household-reports", frozenset({"household.read", "workspace.write"}))
    manager.enable("household-reports")
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Save my open chores to workspace as chores.md",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.action_id == "household-reports.chores_to_workspace"
    assert card.action.arguments == {"target_path": "chores.md"}


def test_deterministic_grocery_report_uses_kitchen_workspace_composition():
    manager = PackManager()
    bundle = next(
        bundle for bundle in reference_bundles() if bundle.manifest.pack_id == "kitchen-reports"
    )
    manager.discover(bundle)
    manager.install("kitchen-reports", frozenset({"kitchen.read", "workspace.write"}))
    manager.enable("kitchen-reports")
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Save my grocery list to workspace as groceries.md",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.action_id == "kitchen-reports.groceries_to_workspace"
...
def test_deterministic_homelab_health_action_accepts_natural_status_question():
    manager = PackManager()
    bundle = next(bundle for bundle in reference_bundles() if bundle.manifest.pack_id == "homelab")
    manager.discover(bundle)
...
def test_deterministic_homelab_services_health_action_accepts_healthy_collection_question():
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="What services are healthy?",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.action_id == "homelab.services.health"
...
def test_deterministic_homelab_restart_strips_terminal_punctuation():
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Restart the service acceptance-plex.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.action_id == "homelab.service.restart"
    assert card.action.arguments == {"service": "acceptance-plex"}
...
def test_deterministic_grocery_message_uses_one_owner_approved_target(monkeypatch):
    monkeypatch.setenv(
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me the grocery list.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.arguments == {
    ...
    assert not isinstance(grounded, Result)
    assert all(
        grounded.action.argument_provenance[key].kind.value == "APPROVED_DEFAULT"
...
def test_deterministic_grocery_message_requires_unambiguous_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"},'
        '{"target":"partner","channel":"sms","account":"household"}]',
    )
    manager = PackManager()
    bundle = next(
        bundle for bundle in reference_bundles() if bundle.manifest.pack_id == "communications"
    ...
    assert _deterministic_composition_action(intent, manager, Context()) is None


def test_deterministic_task_message_uses_one_owner_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"}]',
    )
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me my open tasks.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.arguments["body_source"] == "canonical.tasks"


def test_deterministic_completed_task_message_uses_one_owner_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"}]',
    )
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me my completed tasks.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.arguments["body_source"] == "canonical.completed_tasks"


def test_deterministic_calendar_task_message_uses_one_owner_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"}]',
    )
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me the tasks due before my calendar events.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
...
def test_deterministic_calendar_message_uses_one_owner_approved_target(monkeypatch):
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me my calendar.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.arguments["body_source"] == "canonical.calendar"
    from aegis.reference_interaction import _ground_argument_provenance
...
def test_deterministic_weather_message_uses_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"}]',
    )
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me tomorrow's weather.",
    )
    card = _deterministic_composition_action(intent, manager, Context())
    assert card is not None
    assert card.action.arguments["body_source"] == "public.weather"
    from aegis.reference_interaction import _ground_argument_provenance
...
def test_deterministic_document_search_message_uses_one_owner_approved_target(monkeypatch):
    monkeypatch.setenv(
        "AEGIS_APPROVED_COMMUNICATION_TARGETS",
        '[{"target":"scotty","channel":"sms","account":"household"}]',
    )
    ...
    intent = IntentFrame(
        principal=Principal(id="alice", vault_id="vault"),
        utterance="Text me the document search results for guidance.",
    )
    card = _deterministic_composition_action(
        intent,
        manager,
...
def test_browser_app_calendar_surface_exposes_snapshot_communication():
    app = BrowserApp(
        Principal(id="alice", vault_id="vault"),
        lambda *_args: "unused",
        lambda _current: {"nodes": []},
...
def test_browser_app_household_surface_exposes_grocery_communication():
    app = BrowserApp(
        Principal(id="alice", vault_id="vault"),
        lambda *_args: "unused",
        lambda _current: {"nodes": []},
...
def test_browser_app_calendar_surface_exposes_public_holiday_communication():
    app = BrowserApp(
        Principal(id="alice", vault_id="vault"),
        lambda *_args: "unused",
        lambda _current: {"nodes": []},
...
def test_constellation_state_projects_scoped_active_objective_nodes(monkeypatch):
    ...
    assert {"source": need["id"], "target": "pack-workspace"} in state["edges"]
    assert any(
