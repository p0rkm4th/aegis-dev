...
def test_obligation_focus_read_does_not_render_as_mutation():
    obligation = HouseholdObligation("obligation-1", "Utilities", 120, "bob", False)
    result = resolve_contextual_obligation_focus_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
...
def test_contextual_obligation_focus_accepts_assigned_to_wording():
    obligation = HouseholdObligation("obligation-assignee-synonym", "Utilities", 120, "bob", False)
    result = resolve_contextual_obligation_focus_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="Who is it assigned to?",
        ),
        Context(
            values={
                "canonical_facts": {
...
def test_contextual_event_next_followup_uses_authorized_future_event():
    ...
    assert result.evidence["event"]["title"] == "next event"
...
def test_contextual_event_relative_read_selects_unique_later_event():
    from aegis.household import HouseholdEvent

    result = resolve_contextual_event_relative_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="What about the event after that?",
        ),
        Context(
        ...
        ),
        {
            ...
            "events": (
                ...
                HouseholdEvent(
                    "late", "later event", datetime(2026, 9, 7, 10, tzinfo=timezone.utc)
                ),
            ),
        },
    )
    ...
    assert result is not None
    assert result.evidence["authorized_relative_referent"]["event_id"] == "late"


def test_contextual_event_relative_read_accepts_explicit_before_noun():
    from aegis.household import HouseholdEvent

    result = resolve_contextual_event_relative_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="What about the event before that?",
        ),
        Context(
            values={"canonical_facts": {"event": {"event_id": "middle", "title": "middle"}}},
            sources=("authorized_canonical_result",),
...
def test_contextual_event_relative_read_accepts_show_correction():
    ...
    result = resolve_contextual_event_relative_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="Actually, show the event after that.",
        ),
        Context(
            values={"canonical_facts": {"event": {"event_id": "middle", "title": "middle"}}},
            sources=("authorized_canonical_result",),
...
def test_contextual_task_after_that_accepts_explicit_noun():
    first = {"task_id": "one", "title": "first task", "status": "open"}
    second = {"task_id": "two", "title": "next task", "status": "open"}
    third = {"task_id": "three", "title": "later task", "status": "open"}
    result = resolve_contextual_ordinal_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="What about the task after that?",
        ),
        Context(
            values={
                "canonical_facts": {"task": second},
                "referents": {
                    "those": {
                        "fact_key": "canonical_tasks",
...
def test_contextual_task_after_that_accepts_show_form():
    first = {"task_id": "one", "title": "first task", "status": "open"}
    second = {"task_id": "two", "title": "next task", "status": "open"}
    third = {"task_id": "three", "title": "later task", "status": "open"}
    result = resolve_contextual_ordinal_read(
        IntentFrame(
            principal=Principal(id="alice", vault_id="alice-vault"),
            utterance="Show me the task after that.",
        ),
        Context(
            values={
                "canonical_facts": {"task": second},
