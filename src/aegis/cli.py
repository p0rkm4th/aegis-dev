...
def _deterministic_composition_action(
    ...
    pantry_update_quantity = re.fullmatch(
        r"update pantry item (?P<item_id>[a-zA-Z0-9_.:-]+) named (?P<display_name>.+?) quantity "
        r"(?P<quantity>[0-9]+(?:\.[0-9]+)?)(?: unit (?P<unit>[a-zA-Z0-9_-]+))? version "
        r"(?P<expected_version>[0-9]+)",
        text,
        flags=re.IGNORECASE,
    )
    if pantry_update_quantity is not None:
        pantry_card = manager.action_card("kitchen", "kitchen.pantry.update")
        if pantry_card is not None:
            display_name = " ".join(pantry_update_quantity.group("display_name").split())
            quantity = float(pantry_update_quantity.group("quantity"))
            if quantity.is_integer():
                quantity = int(quantity)
            if display_name:
                pantry_arguments = {
                    "item_id": pantry_update_quantity.group("item_id"),
                    "display_name": display_name,
                    "quantity": quantity,
                    "expected_version": int(pantry_update_quantity.group("expected_version")),
                }
                unit = pantry_update_quantity.group("unit")
                if unit:
                    pantry_arguments["unit"] = unit
                return pantry_card.model_copy(
                    update={
    ...
    air_quality_report = re.fullmatch(
        r"save (?:the )?(?:current )?air quality at "
        r"(?P<latitude>-?[0-9]{1,2}(?:\.[0-9]{1,6})?),\s*"
        r"(?P<longitude>-?[0-9]{1,3}(?:\.[0-9]{1,6})?) as "
        r"(?P<target_path>[a-zA-Z0-9][a-zA-Z0-9_./-]{0,120})",
        text,
    ...
    if air_quality_report is not None:
        card = manager.action_card(
            "air-quality-reports", "air-quality-reports.current.to_workspace"
        )
        if card is None:
            return None
        return card.model_copy(
            update={
                "action": card.action.model_copy(
                    update={
                        "arguments": {
                            "latitude": float(air_quality_report.group("latitude")),
                            "longitude": float(air_quality_report.group("longitude")),
                            "target_path": air_quality_report.group("target_path"),
                        }
                    }
                )
            }
    ...
    chores_send_me = re.fullmatch(
        r"(?:send|text) me (?:my )?(?:open )?chores[?!.,]?",
        text,
        flags=re.IGNORECASE,
    )
    if chores_send_me is not None:
        try:
            approved_targets = configured_communication_targets()
        except ValueError:
            approved_targets = frozenset()
                    ...
                    "action": card.action.model_copy(
                        update={
                            "arguments": {
                                ...
                                "channel": channel,
                                "account": account,
                                "body_source": "canonical.chores",
                            }
                        }
                    )
                }
            )
    obligations_send_me = re.fullmatch(
        r"(?:send|text) me (?:my )?(?:open )?obligations[?!.,]?",
        text,
        flags=re.IGNORECASE,
    )
    if obligations_send_me is not None:
        try:
            approved_targets = configured_communication_targets()
        except ValueError:
            approved_targets = frozenset()
    ...
    completed_task_send_me = re.fullmatch(
        r"(?:send|text) me (?:my )?(?:completed|done|finished) tasks[?!.,]?",
        text,
        flags=re.IGNORECASE,
    )
    if completed_task_send_me is not None:
        try:
            approved_targets = configured_communication_targets()
        except ValueError:
            approved_targets = frozenset()
                    ...
                    "action": card.action.model_copy(
                        update={
                            "arguments": {
                                ...
                                "channel": channel,
                                "account": account,
                                "body_source": "canonical.completed_tasks",
                            }
                        }
                    )
                }
