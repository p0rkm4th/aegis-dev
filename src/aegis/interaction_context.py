...
def compact_context_evidence(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("task", "event", "obligation", "chore"):
    ...
    if result.state.value == "blocked":
        ...
        if isinstance(those, dict):
            ...
            if fact_key in {
                ...
                # canonical referents already authorized on the prior turn.
                # Never overwrite fresh evidence from a completed follow-up.
